...
if (!LINKEDIN_EMAIL || !LINKEDIN_PASSWORD) {
  throw new Error(
    "Missing required environment variables: LINKEDIN_EMAIL and LINKEDIN_PASSWORD"
  );
}

/**
 * @interface LinkedInJobApplication
 * @description Defines the structure for a LinkedIn job application.
 */
interface LinkedInJobApplication {
  jobUrl: string;
  jobTitle: string;
 ...
 */
async function delay(min: number = 1000, max: number = 3000): Promise<void> {
...
async function loginToLinkedIn(page: any) {
  try {
    ...
    async function findLinkedInElement(
      selectors: string[],
      elementName: string
    ) {
    ...
    const emailInput = await findLinkedInElement(emailSelectors, "email input");
    ...
    const passwordInput = await findLinkedInElement(
      passwordSelectors,
      "password input"
    );
    ...
    const submitButton = await findLinkedInElement(
      submitSelectors,
      "submit button"
    );
    ...
    for (const selector of challengeSelectors) {
      try {
        if (await page.locator(selector).isVisible({ timeout: 3000 })) {
          console.warn(
            "⚠️ LinkedIn challenge/verification detected. Manual intervention required."
          );
          console.warn(
    ...
    await page.screenshot({ path: "linkedin-login-error.png" });
    throw error;
  }
}
 ...
 */
async function checkAnySelector(
  page: Page,
  selectors: string[]
): Promise<boolean> {
  for (const selector of selectors) {
 ...
 */
async function searchLinkedInJobs(page: Page): Promise<any[]> {
  try {
    console.log("🔍 Navigating to LinkedIn Jobs...");

    await page.goto(
      "https://www.linkedin.com/jobs/search-results/?f_AL=true&f_TPR=r86400&geoId=114806696&keywords=MERN stack developer OR Node.js developer OR React developer OR Full stack developer",
      {
        waitUntil: "domcontentloaded",
        timeout: 60000,
      }
 ...
 */
async function applyJobFilters(page: Page): Promise<void> {
  try {
    // Easy Apply filter
    ...
    if (await dateButton.isVisible({ timeout: 5000 })) {
      ...
      const pastWeekOption = page.locator(
        'input[value="r604800"], label:has-text("Past week") input'
      );
      if (await pastWeekOption.isVisible({ timeout: 3000 })) {
        await pastWeekOption.check();
 ...
 */
async function extractJobListings(page: Page): Promise<any[]> {
  const allJobs: any[] = [];
  let pageNumber = 1;
      ...
      jobCards.forEach((card) => {
        try {
          const titleElement = card.querySelector(
            ".job-card-list__title strong, .job-card-list__title a"
          );
          const companyElement = card.querySelector(
            ".job-card-container__primary-description, .job-card-container__company-name a"
          );
          ...
          const easyApplyBadge = card.querySelector(
            ".job-card-container__apply-method, .job-card-list__easy-apply"
          );
 ...
 */
async function applyToLinkedInJob(
  page: Page,
  jobUrl: string,
  jobTitle: string,
  company: string
  ...
  try {
    ...
    const easyApplyButton = await findElement(
      page,
      easyApplySelectors,
      "Easy Apply button"
    );
    ...
    const result = await handleApplicationFlow(page, jobTitle, company);
    ...
    await page.screenshot({
      path: `linkedin-apply-error-${jobTitle.replace(
        /[^a-z0-9]/gi,
        "_"
      )}-${Date.now()}.png`,
    });
...
async function handleApplicationFlow(
  ...
  try {
    ...
    while (currentStep <= maxSteps && !isCompleted) {
      ...
      const isModalPresent = await page
      ...
      if (!buttonFound) {
        console.log(
          "⚠️ No action buttons found, application may be incomplete"
        );
        break;
      }
    ...
    if (isCompleted) {
      return await verifyApplicationSuccess(page, jobTitle, company);
    }
    ...
    console.error(
      `❌ Error in application flow for ${jobTitle}:`,
      error.message
    );
    return {
      success: false,
...
async function handleResumeUpload(page: Page): Promise<void> {
  try {
    ...
    if (!fs.existsSync(resumePath)) {
      console.log(
        "ℹ️ Resume.pdf not found in current directory, skipping upload"
      );
      return;
    }
 ...
 */
async function handleApplicationQuestions(page: Page): Promise<void> {
  try {
    ...
    const textInputs: any = await page.$$(
      'input[type="text"], input[type="tel"], input[type="email"], textarea'
    );
    for (const input of textInputs) {
      try {
        const labelText = await getInputLabel(page, input);
    ...
    await handleMultipleChoiceQuestions(page);
  } catch (error: any) {
 ...
 */
async function getInputLabel(page: Page, input: any): Promise<string> {
  try {
      ...
      () =>
        page.evaluate((el: Element) => {
          const container = el.closest(
            ".form-element, .artdeco-text-input, .fb-single-line-text"
          );
          return (
            container
 ...
 */
async function handleMultipleChoiceQuestions(page: Page): Promise<void> {
  try {
    // Get all labels that might contain radio/checkbox options
    const labels: any = await page.$$("label");
      ...
      try {
        ...
        const shouldSelect =
          ...
          (normalizedText.includes("sponsorship") &&
            negativeForSponsorship.some((indicator) =>
              normalizedText.includes(indicator)
            ));
        ...
        if (shouldSelect) {
          ...
          if (input && (await input.isVisible({ timeout: 1000 }))) {
            await input.check();
...
async function verifyApplicationSuccess(
  ...
  const isModalClosed = !(await page
 ...
 */
function shouldApplyToJob(job: any): boolean {
  ...
  const isPreferredLocation = preferredLocations.some((loc) =>
 ...
 */
async function saveApplicationResults(
 ...
 */
async function scrapeAndApplyLinkedInJobs(
  useExistingBrowser: boolean = true
): Promise<ApplicationResult[]> {
  ...
  try {
    if (useExistingBrowser) {
      ...
      try {
        ...
        console.log(
          "⚠️ Failed to connect to existing browser, launching new instance..."
        );
        console.log(
  ...
  try {
    ...
    const jobs = await searchLinkedInJobs(page);
    ...
    console.log(
      `📋 Processing ${jobs.length} jobs (max applications: ${maxApplications})...`
    );
    ...
    for (const job of jobs) {
      try {
        if (appliedCount >= maxApplications) {
          console.log(
            `🛑 Reached maximum applications limit (${maxApplications})`
          );
          break;
        }
        ...
        if (result.success) {
          appliedCount++;
          console.log(
            `✅ Successfully applied! (${appliedCount}/${maxApplications})`
          );
        } else {
        ...
        console.log(
          `⏳ Waiting ${Math.round(
            waitTime / 1000
          )} seconds before next application...`
        );
        await delay(waitTime, waitTime + 5000);
      } catch (error: any) {
        console.error(
          `❌ Unexpected error processing job "${job.title}":`,
          error.message
        );
    ...
    await saveApplicationResults(applicationResults);
    ...
    if (successful > 0) {
      ...
      applicationResults
        ...
        .forEach((result, index) => {
          console.log(
            `   ${index + 1}. ${result.jobTitle} at ${result.company}`
          );
        });
    }
 ...
 */
async function connectToExistingBrowser(): Promise<{
  browser: Browser;
 ...
 */
function printBrowserSetupInstructions(): void {
  console.log("\n" + "=".repeat(60));
  console.log("🌐 BROWSER SETUP FOR AUTOMATION");
  console.log("=".repeat(60));
  console.log(
    "To use your existing browser, you need to start it with debugging enabled:"
  );
  console.log("");
  console.log("📌 For Chrome/Chromium:");
  console.log(
    "   Windows: chrome.exe --remote-debugging-port=9222 --user-data-dir=remote-profile"
  );
  console.log(
    "   macOS: /Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port=9222 --user-data-dir=remote-profile"
  );
  console.log(
    "   Linux: google-chrome --remote-debugging-port=9222 --user-data-dir=remote-profile"
  );
  console.log("");
  console.log("📌 For Microsoft Edge:");
  console.log(
    "   Windows: msedge.exe --remote-debugging-port=9222 --user-data-dir=remote-profile"
  );
  console.log("");
  console.log("💡 Tips:");
  console.log("   - Close all existing browser instances first");
  console.log(
    "   - The --user-data-dir creates a separate profile for automation"
  );
  ...
  console.log("");
  console.log(
    "🔗 Alternative: Set useExistingBrowser to false to launch new browser"
  );
  console.log("=".repeat(60));
}
